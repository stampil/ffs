...
function calendar_display_event()
{
	...
	$rsvp_color_yes = get_calendar_config_value("rsvp_color_yes", '');
	$rsvp_color_no = get_calendar_config_value("rsvp_color_no", '');
	$rsvp_color_maybe = get_calendar_config_value("rsvp_color_maybe", '');
	...
	if( $event_id > 0 )
	{
		...
		if ( !$auth->acl_get('u_calendar_view_events') )
		{
            ...
            login_box('', $user->lang['LOGIN_EXPLAIN_VIEW_EVENT']);
		}
		...
		if( $event_data['track_rsvps'] == 1 )
		{
			...
			if( $submit )
			{
				...
				// negative rsvp counts are not allowed.
				if( $new_rsvp_count < 0 )
				{
					$new_rsvp_count = 0;
				}
				...
				// trigger error to prevent multiple rsvps on refresh:
				...
				if( $mode == 'edit' )
				{
					$message = $user->lang['RSVP_UPDATED'] . '<br /><br />' . sprintf($user->lang['VIEW_EVENT'], '<a href="' . $view_event_url . '">', '</a>');

				}
				else
				{
					$message = $user->lang['RSVP_SAVED'] . '<br /><br />' . sprintf($user->lang['VIEW_EVENT'], '<a href="' . $view_event_url . '">', '</a>');
				}
...
function display_users_next_events_for_x_days( $x, $user_id )
{
	...
	if ( $auth->acl_get('u_calendar_view_events') )
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if( $row['rsvp_val'] == 0 )
			{
				$events['COLOR_RSVP'] = $rsvp_color_yes;
				$events['VALUE_TXT'] = $user->lang['YES'];
			}
			else if( $row['rsvp_val'] == 1 )
			{
				$events['COLOR_RSVP'] = $rsvp_color_no;
				$events['VALUE_TXT'] = $user->lang['NO'];
			}
			else
			{
				$events['COLOR_RSVP'] = $rsvp_color_maybe;
		...
		case 1:
			...
			else
			{
				...
				for( $i = 0; $i < $num_groups; $i++ )
				{
					...
					$db->sql_freeresult($result);
					$temp_list = (($group_data['group_type'] == GROUP_SPECIAL) ? $user->lang['G_' . $group_data['group_name']] : $group_data['group_name']);
					$temp_url = append_sid("{$phpbb_root_path}memberlist.$phpEx", "mode=group&amp;g=".$group_data['group_id']);
					$temp_color_start = "";
					$temp_color_end = "";
					if( $group_data['group_colour'] !== "" )
					{
			...
			case 1:
				...
				while( must_find_next_occ( $row, $end_populate_limit ))
				{
				    ...
				    $sql = 'INSERT INTO ' . CALENDAR_EVENTS_TABLE . ' ' . $db->sql_build_array('INSERT', array(
							...
							'group_id_list'         => (string) $row['group_id_list'],
